...
export default class Register extends Component {
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={{width:width*0.9,flexDirection:'row'}}>
                    ...
                    {/*获取验证码*/}
                    <TouchableOpacity onPress = {() => this.__getTest()}>
    ...
    __getTest=()=>{
        ...
        if(this.state.username.length == 11 && flag){
            ...
            formData.append("action","messageSend");
    ...
    __register=()=>{
        ...
        if(this.state.username.length == 11){
            let formData = new FormData();
            formData.append("action","registerWithNum");
            formData.append("phone",this.state.username);
            formData.append("deviceid",DeviceInfo.getDeviceId());
            formData.append("password",this.state.password);
            formData.append("code",this.state.test);
            fetch(url,{
                    method:'POST',
                    headers:{
                },

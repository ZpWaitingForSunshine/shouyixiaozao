...
export default class AnswerView extends Component {
    ...
    renderScrollItem = () => {
        ...
        // 遍历创建组件
        for(var i=0; i<JSON.parse(this.props.idtree).length; i++){
            itemArr.push(
                <View 
        ...
        if ((i == this.state.activePage)) {
            ...
            return(
                <AnswerCell
                    ...
                    keykey = {i}
                    style={{width:width,height:height}}
                    id={JSON.parse(this.props.idtree)[i]}
                    length = {JSON.parse(this.props.idtree).length}>
                </AnswerCell>)
        }
            ...
            AsyncStorage.getItem('phone').then(
                (result)=>{
                    if(result){
                        ...
                        formData.append("tx_id",JSON.parse(this.props.idtree)[this.state.activePage]);
    ...
    shareToWY = () => {
        var num = JSON.parse(this.props.idtree)[this.state.activePage]
    ...
    isCollections =(currentPage)=>{
        ...
        formData.append("tx_id",JSON.parse(this.props.idtree)[currentPage]);
